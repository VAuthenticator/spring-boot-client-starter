...
public class OAuth2RefreshableTokenResolver implements OAuth2TokenResolver {
    ...
    private final static Logger logger = LoggerFactory.getLogger(OAuth2RefreshableTokenResolver.class);
    ...
    public String tokenFor(OAuth2AuthenticationToken currentUser) {
        ...
        logger.debug("getAuthorizedClientRegistrationId : " + currentUser.getAuthorizedClientRegistrationId());
        ...
        if (isExpired(client.getAccessToken())) {
            logger.debug("token have to refresh");
    ...
    private void refreshToken(OAuth2AuthorizedClient client, OAuth2AuthenticationToken currentUser) {
        ...
        if (oAuth2AccessTokenResponse == null || oAuth2AccessTokenResponse.getAccessToken() == null) {
            logger.debug("refresh goes in error: oAuth2AccessTokenResponse is null");
            return;
        }

        logger.debug("oAuth2AccessTokenResponse.getRefreshToken(): " + oAuth2AccessTokenResponse.getRefreshToken());
        logger.debug("oAuth2AccessTokenResponse.getAccessToken(): " + oAuth2AccessTokenResponse.getAccessToken());
    ...
    private OAuth2AccessTokenResponse refreshTokenClient(OAuth2AuthorizedClient currentClient) {
        ...
        try {
            ...
            logger.error(String.format("Unable to refresh token %s", e.getMessage()), e);
    ...
    private boolean isExpired(OAuth2AccessToken accessToken) {
        ...
        logger.debug("now.isAfter(expiresAt) " + now.isAfter(expiresAt.minus(amountToSubtract)));
